...
from identificadores import SiglaCompanhiaAerea, RegistroDeAeronave, CodigoVoo, RegistroDeViagem, \
    GeradorDeRegistroDeViagem, SiglaAeroporto, DocumentoCliente, GeradorDeRegistroDePassagem, CodigoDoAssento, \
    RegistroDePassagem
...
class CompanhiaAerea(Persist):
    ...
    def _encontrar_melhor_voo(self, data: Data, aeroporto_de_saida: SiglaAeroporto,
        ...
        if len(voos_sem_conexao) != 0:
            ...
            for codigo_voo in voos_sem_conexao:
                ...
                tarifa = voo.calcula_tarifa(len(franquias.franquias), self.tarifa_franquia)
        ...
        for (codigo_voo_intermediario, codigo_voo_final) in pares_de_voos:
            ...
            tarifa_intermediario = voo_intermediario.calcula_tarifa(len(franquias.franquias), self.tarifa_franquia)
            tarifa_final = voo_final.calcula_tarifa(len(franquias.franquias), self.tarifa_franquia)
    ...
    def comprar_passagem(self, id_cliente: DocumentoCliente, data: Data, aeroporto_de_saida: SiglaAeroporto, aeroporto_de_chegada: SiglaAeroporto, franquias: FranquiasDeBagagem, assento: Optional[CodigoDoAssento]) -> Optional[RegistroDePassagem]:
        voos = self._encontrar_melhor_voo(data, aeroporto_de_saida, aeroporto_de_chegada, franquias)
        if len(voos) == 0:
            return None
        ...
        for viagem_factory in viagem_factories:
            ...
            valor = viagem_factory.reservar_assento(registro_passagem, franquias, assento_desejado)
            valor_total += valor
